...
class DateImprintProcessor @Inject constructor(
    ...
    // FIX: cache one SimpleDateFormat per style — it's a heavyweight object with
    // locale/calendar state. Creating a new one per shot wastes ~0.5ms each time.
    private val dateFormatters: Map<DateImprintStyle, SimpleDateFormat> by lazy {
        mapOf(
            DateImprintStyle.CLASSIC    to SimpleDateFormat("MMM dd ''yy",  Locale.US),
            DateImprintStyle.NUMERIC_US to SimpleDateFormat("MM/dd/yy",     Locale.US),
            DateImprintStyle.NUMERIC_EU to SimpleDateFormat("dd.MM.yy",     Locale.US),
            DateImprintStyle.YEAR_FIRST to SimpleDateFormat("''yy MMM dd",  Locale.US),
            DateImprintStyle.WITH_TIME  to SimpleDateFormat("dd MMM HH:mm", Locale.US),
            DateImprintStyle.YY_MM_DD   to SimpleDateFormat("''yy MM dd",   Locale.US),
            DateImprintStyle.YYYY_MM_DD to SimpleDateFormat("yyyy MM dd",   Locale.US),
        )
    }

    private fun formatDate(style: DateImprintStyle): String {
        val now = Date()
        val fmt = dateFormatters[style] ?: SimpleDateFormat("MM/dd/yy", Locale.US)
        return when (style) {
            DateImprintStyle.CLASSIC,
            DateImprintStyle.YEAR_FIRST,
            DateImprintStyle.WITH_TIME  -> fmt.format(now).uppercase()
            else                        -> fmt.format(now)
        }
    }
}
