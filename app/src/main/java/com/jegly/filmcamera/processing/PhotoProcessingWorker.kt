...
class PhotoProcessingWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val lutFilePath = inputData.getString(KEY_LUT_FILE_PATH)?.takeIf { it.isNotEmpty() }
        ...
        // Custom (imported) LUTs aren't in the static catalog — rebuild a neutral film
        // that carries the imported LUT path so the same grade applies to the photo.
        val film = if (lutFilePath != null) {
            FilmStock(
                id = filmId,
            ...
            FilmCatalog.findById(filmId) ?: run {
                rawFile.delete()
                return Result.failure()
            }
        }
    ...
    companion object {
        ...
        const val KEY_LUT_FILE_PATH = "lut_file_path"
